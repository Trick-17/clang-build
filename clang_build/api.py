...
def get_project(path, working_directory, environment, parent=None):
    ...
    module_file_path = path.resolve() / "clang_build_project.py"
    module_name = "clang_build_project"
    ...
    clang_build_project = importlib_util.module_from_spec(module_spec)
    module_spec.loader.exec_module(clang_build_project)

    if clang_build_project.get_project is None:
        raise _ApiError(f'Module "{module_name}" in "{path.resolve()}" does not contain a `get_project` method')

    return clang_build_project.get_project(working_directory, environment, parent=parent)