...
class TestClangBuild(unittest.TestCase):
    def test_hello_world_mwe(self):
        ...
        try:
            output = (
                subprocess.check_output(
                    ["./build/default/bin/main"], stderr=subprocess.STDOUT
                )
                .decode("utf-8")
                .strip()
            )
        ...
        for command in compile_commands:
            self.assertEqual(
                str(_Path("test/mwe/hello.cpp").resolve()),
                str(_Path(command["file"]).resolve()),
            )
            self.assertTrue(
                str(_Path("./build/default/obj/hello.o").resolve())
                == str(_Path(command["output"]).resolve())
                or str(_Path("./build/default/dep/hello.d").resolve())
                == str(_Path(command["output"]).resolve())
            )
    ...
    def test_build_types(self):
        for build_type in ["release", "relwithdebinfo", "debug", "coverage"]:
            ...
            try:
                output = (
                    subprocess.check_output(
                        [f"./build/{build_type}/bin/main"], stderr=subprocess.STDOUT
                    )
                    .decode("utf-8")
                    .strip()
                )
                ...
                self.fail(
                    f'Could not run compiled program with build type "{build_type}". Message:\n{e.output}'
                )
    ...
    def test_script_call(self):
        try:
            subprocess.check_output(
                ["clang-build", "-d", "test/mwe", "-V"], stderr=subprocess.STDOUT
            )
        except subprocess.CalledProcessError as e:
    ...
    def test_toml_mwe(self):
        ...
        try:
            output = (
                subprocess.check_output(
                    ["./build/default/bin/runHello"], stderr=subprocess.STDOUT
                )
                .decode("utf-8")
                .strip()
            )
    ...
    def test_subproject(self):
        ...
        try:
            output = (
                subprocess.check_output(
                    ["./build/myexe/default/bin/runLib"], stderr=subprocess.STDOUT
                )
                .decode("utf-8")
                .strip()
            )
    ...
    def test_boost_filesystem(self):
        ...
        try:
            output = (
                subprocess.check_output(
                    ["./build/myexe/default/bin/myexe", "build"],
    ...
    def test_c_library(self):
        ...
        try:
            output = (
                subprocess.check_output(
                    ["./build/myexe/default/bin/myexe"], stderr=subprocess.STDOUT
                )
                .decode("utf-8")
                .strip()
            )
    ...
    def test_build_all(self):
        ...
        try:
            output = (
                subprocess.check_output(
                    ["./build/qhull/qhull-executable/default/bin/qhull", "-V"],
                    stderr=subprocess.STDOUT,
                )
                .decode("utf-8")
                .strip()
    ...
    def test_platform_flags(self):
        ...
        try:
            output = (
                subprocess.check_output(
                    ["./build/default/bin/myexe"], stderr=subprocess.STDOUT
                )
                .decode("utf-8")
                .strip()
            )
        ...
        if _platform == "linux":
            ...
            raise RuntimeError(
                "Tried to run test_platform_flags on unsupported platform " + _platform
            )
    ...
    def test_pybind11(self):
        ...
        pylib_dir = os.path.abspath(
            os.path.join(
                "build",
                "pylib",
                "default",
                toolchain.LLVM.PLATFORM_DEFAULTS[_platform][
        ...
        try:
            ...
            if os.path.exists(pylib_dir):
                print(
                    f'Expected location "{pylib_dir}" contains: {os.listdir(pylib_dir)}'
                )
            else:
